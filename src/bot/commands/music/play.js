	...
	async exec(message, { query, unshift }) {
		if (!message.member.voice || !message.member.voice.channel) {
			...
			if (Boolean(message.guild.me.voice.channel.members.filter(m => !m.user.bot).size > 1)) {
				return message.channel.send(`You need to be listening in **${message.guild.me.voice.channel.name}** to do queue songs.`);
			}
		}
		...
		if (['TRACK_LOADED', 'SEARCH_RESULT'].includes(res.loadType)) {
			...
			return message.channel.send(`Are you sure \`\`${query.replace('ytsearch', '')}\`\` is a thing to play? I couldn't find it!`);
		}
		if (!queue.player.playing && !queue.player.paused) await queue.start();

		return message.channel.send(`**Queued up:** \`${msg}\``);
	}
}
